...
基础字号 17px，符合 Apple HIG 推荐的移动端阅读标准。中文字距 0.01em。**行高分四档**：标题 1.22（紧凑）、列表 1.5（紧凑，让列表项聚合）、次要信息 1.65（图注脚注）、正文 1.75（宽松，适合长时间阅读）——区分「行内密度」和「段落呼吸」。
...
<ThemeToggle variant="header" class="inline-block" />
...
间距基准沿 4px 台阶走到 48px，覆盖从 hairline 到 section 分隔的所有场景。

<SpacingShowcase />

在排版层面之上，还额外抽了一组**块级节奏**的语义 token，用来区分「紧邻聚合」和「段落呼吸」：

- `--space-block-tight`（4px）—— 列表项之间、紧挨的小块；配合 `--line-height-compact` (1.5) 让列表视觉上聚合
- `--space-block-base`（16px）—— 不同类型块之间的默认间距，比如段落与列表的过渡
- `--space-block-loose`（24px）—— 段落与段落之间，强调思绪的停顿

比例大概是 1 : 4 : 6，所以扫一眼就能分出「这是一条列表」还是「这是新的一段」。`.prose` 和评论区都统一用这套节奏，避免两套规则飘忽不定。
