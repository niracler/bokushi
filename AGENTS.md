...
- `src/pages/` holds route-level Astro files; `src/layouts/` shares shells; `src/components/` keeps reusable UI (Header, Footer, BaseHead, Remark42).
- Content lives in `src/content/` with `content.config.ts` validating frontmatter; prefer kebab-case slugs. Shared static assets go in `public/`; `dist/` is build output and should remain untracked.
- Styling is centralized in `src/styles/` (tokens plus Tailwind 4 layers); utilities/scripts live in `src/utils/`, `src/types/`, and `src/scripts/`; site metadata sits in `src/consts.ts`.
...
- Quality: `pnpm lint` (Biome) and `pnpm format`. Run `pnpm astro check` when adding TS/MDX.
...
- Keep Astro/TS lean: derive props from data, prune unused imports/variables manually.

## Testing Guidelines
- Automated tests are not set up; Playwright is available if you add browser specs. Name them `*.spec.ts` under `tests/` and keep headless-friendly.
...
- **PR process**: branch from `main`, open a PR with intent + test plan, update docs when needed, prefer squash-and-merge. Include before/after screenshots for visual work and call out config/env changes.
